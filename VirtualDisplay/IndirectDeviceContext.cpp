...
IndirectDeviceContext::IndirectDeviceContext(WDFDEVICE device): WdfDevice(device)
{
	Adapter = {};
}
...
void IndirectDeviceContext::InitAdaptr()
{
    // ==============================
    ...
    AdapterInit.WdfDevice = WdfDevice;
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        Adapter = AdapterInitOut.AdapterObject;
...
void IndirectDeviceContext::FinishInit(UINT index)
{
    ...
    if (index >= s_SampleMonitorsCount)
    {
    ...
    else
    {
        ...
        MonitorInfo.MonitorDescription.pData = const_cast<BYTE*>(s_SampleMonitors[index].pEdidBlock);
    }
